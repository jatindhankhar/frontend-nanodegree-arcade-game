...
let buginfo = document.getElementById("buginfo");

for(let i=0;i<6;i++)
   buginfo.appendChild(document.createElement("p"));
// Sprite Margins 
...
class Enemy {
    ...
    update(dt) {
        ...
        buginfo.children[this.bugnumber].textContent = `Bug number: ${this.bugnumber} --> ${JSON.stringify(this.getCurrentTile())}`
    }
    ...
    getCurrentTile()
    {
        ...
        if(floor == 0)
            tiles = {'first' : 0,'second': 0};
        else
            tiles = {'first': floor, 'second': ceil}

        tiles['first'] += Math.floor((this.y-enemyYMargin)/83) * 5;
        tiles['second'] += Math.floor((this.y-enemyYMargin)/83) * 5;
        return tiles;
...
class Player {
    ...
    update(dx=0,dy=0){
        // Check for bounds 
        if(this.x + dx < 0 || this.x + dx > 4*101 || this.y + dy < -11 || this.y + dy > 5*83)
           {
    ...
    getCurrentTile()
    {
        ...
        // Tile number y = position/83 (1..n)
        // Tile number x = position/101 (0..n)
        // Final position y_pos * no_of_cols + x
        if (this.y <= 0) return -1;
        else
        return Math.floor((this.y-playerYMargin)/83) * 5 + Math.floor(Math.floor(this.x) / 83);
    }
...
for(let i=0,row=1;i<numEnemies;i++){
    ...
    if(row >= 4) row = 1;
    allEnemies.push(
        new Enemy(x=getRandomIntInclusive(0,2*101), // Place insect on first two tiles of a row 
            y=row*83,
...
let player = new Player(x=0,y=404);
